...
async function memberExists(roomId){
  ...
  var memberExists;
  database().ref(`/Rooms/${roomId}/votes/${instance}`)
  .once('value')
  .then(snapshot => {
    if(snapshot.exists()){
      memberExists = true;
...
async function enterRoomCheck(roomId){
  ...
  if(!memberExistsFlag){
    database().ref(`/Rooms/${roomId}/size`).once('value').then(snapshot => {
      if(snapshot.exists){
...
function memberCountCheck(roomId){
  ...
  return reference.transaction(size => {
    if(size.currentCount <size.maxCount){
...
function voteToResult(deltaOBJ,roomId){
  ...
  return reference.transaction(results => {
    ...
    for(var key in deltaOBJ.distance){
      newResult.distance[key] = newResult.distance[key]+deltaOBJ.distance[key];
    }
